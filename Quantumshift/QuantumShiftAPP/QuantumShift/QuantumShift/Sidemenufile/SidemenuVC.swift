...
class SidemenuVC: UIViewController {
    ...
    var arrImg = ["feedback-9" , "send-5" ,"internet", "terms-and-conditions-3","help-2","star-3","information-3"]
    override func viewDidLoad() {
        ...
        tblSidemenu.layer.transform = CATransform3DMakeScale(1.1, 0.75, 0.1)
        UIView.animate(withDuration: 1.0,animations: { [self] in
            tblSidemenu.layer.transform = CATransform3DMakeScale(1.0, 1.0,2.0)
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if indexPath.row == 0{
            ...
            if let link = NSURL(string: "https://www.apple.com") {
                    let objectsToShare = ["HemoConverter Plus", link] as [Any]
...
//            
//            let url = URL(string: "https://unitslab.com/")
//                           if UIApplication.shared.canOpenURL(url!) {
//                               UIApplication.shared.open(url!, options: [:], completionHandler: nil)
//                               
//                               UIApplication.shared.open(url!, options: [:], completionHandler: { (success) in
